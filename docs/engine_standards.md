...
- **สิ่งที่ระบบปัจจุบันทำอยู่ (`main` Branch - Hybrid Approach):** อนุญาตให้ Pad ของผู้เล่นขยับหน้าจอตามปุ่มที่กดได้ทันที (True Local State Prediction) โดยไม่ต้องรอให้เซิร์ฟเวอร์ตอบกลับ จากนั้นจึงส่งพิกัด X ท่ีคำนวณได้ไปให้เซิร์ฟเวอร์ยอมรับ 
...
> 💡 **สำหรับผู้ที่ต้องการศึกษาการ implement แบบ Strict Server Authoritative:** โปรเจกต์นี้มี Branch `professional-netcode` ที่แสดงให้เห็นถึงการ implement แบบเต็มรูปแบบ พร้อม Sequence Number Tracking, Server Reconciliation, และ Input Re-simulation ซึ่งเป็นมาตรฐานที่ใช้ในเกม AAA ระดับโลก
